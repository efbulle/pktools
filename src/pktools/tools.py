...
def zones_homogenes(
    ...
    all_pks = [
        df.select(*on, *pk_lbls).unpivot(index=on, value_name="pk").drop("variable")
        for df in df_list
    ]
...
def calcule_cc(
    ...
    start, end = pk_lbls
    if by is None:
        by_cols = [col for col in df.columns if col not in pk_lbls]
    ...
    return (
        df.sort(*by_cols, start)
        .with_columns(pl.col(end).cum_max().over(by_cols).alias("running_max_end"))
        .with_columns(
            (
                pl.when(pl.col(start) > pl.col("running_max_end").shift().over(by_cols))
                .then(1)
                .otherwise(0)
            ).alias("new_group")
        )
        .with_columns(pl.cum_sum("new_group").over(by_cols).alias("group_id"))
        .group_by(*by_cols, "group_id", maintain_order=True)
        .agg(
